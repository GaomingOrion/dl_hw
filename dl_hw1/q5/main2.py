...
class Model(BaseModel):
    def _graph_kernel(self, inp):
        x = self._fc_layer(name='fc1', inp=inp, units=2000, dropout=0.8)
        x = tf.nn.relu(x, name='ac1')
        x = self._fc_layer(name='fc2', inp=x, units=1000, dropout=0.8)
        x = tf.nn.relu(x, name='ac2')
        logits = self._fc_layer(name='fc_final', inp=x, units=10, dropout=0.8)
        return logits
