...
class Dataset:
    def __init__(self, dataset_name='train', read_from_pkl=True):
        ...
        if not read_from_pkl:
            ...
            with open(os.path.join(self.data_path, 'X_2.pkl'), 'wb') as f:
                pickle.dump(self.X, f)
            with open(os.path.join(self.data_path, 'X_black_2.pkl'), 'wb') as f:
                pickle.dump(self.X_black, f)
            with open(os.path.join(self.data_path, 'labels_2.pkl'), 'wb') as f:
                pickle.dump(self.labels, f)
        else:
            with open(os.path.join(self.data_path, 'X_2.pkl'), 'rb') as f:
                self.X = pickle.load(f)
            with open(os.path.join(self.data_path, 'X_black_2.pkl'), 'rb') as f:
                self.X_black = pickle.load(f)
            with open(os.path.join(self.data_path, 'labels_2.pkl'), 'rb') as f:
