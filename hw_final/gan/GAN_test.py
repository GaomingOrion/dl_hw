...
class Model:
    ...
    def gen(self, img_bw, scope='gen'):
        with tf.variable_scope(scope):
            ...
            with tf.variable_scope('conv_transpose'):
                ...
                net_out = slim.conv2d(conv7, 2, [3, 3], scope='out', activation_fn=tf.nn.relu)
        return net_out
        ...
        with tf.variable_scope(scope):
            with tf.variable_scope('conv'):
                with slim.arg_scope([slim.conv2d], activation_fn=tf.nn.leaky_relu):
                    ...
                    #conv3 = slim.conv2d(conv3, 128, [3, 3], scope='conv3_2')
                    pool3 = slim.max_pool2d(conv3, [2, 2], scope='pool3')
                    conv4 = slim.conv2d(pool3, 256, [1, 1], scope='conv4_1')
                    #conv4 = slim.conv2d(conv4, 256, [3, 3], scope='conv4_2')
    ...
    def build_train_graph(self):
        ...
        # logits_real = self.dis(2*self._image_bw-1, 2*self._image_ab-1, 'dis_real')
        # logits_fake = self.dis(2*self._image_bw-1, gen_out, 'dis_fake')
