...
class Model:
    ...
    def build_infer_graph(self):
        with tf.variable_scope('conv_job1'):
            with slim.arg_scope([slim.conv2d], normalizer_fn=None,
                ...
                conv0_1 = slim.conv2d(self._image_bw, 32, [3, 3], scope='conv1_1')
                conv0_1 = slim.conv2d(conv0_1, 32, [3, 3], scope='conv1_2')
                pool0_1 = slim.max_pool2d(conv0_1, [2, 2], scope='pool1')
                conv0_2 = slim.conv2d(pool0_1, 64, [3, 3], scope='conv2_1')
                conv0_2 = slim.conv2d(conv0_2, 64, [3, 3], scope='conv2_2')
                pool0_2 = slim.max_pool2d(conv0_2, [2, 2], scope='pool2')
                conv0_3 = slim.conv2d(pool0_2, 128, [3, 3], scope='conv3_1')
                conv0_3 = slim.conv2d(conv0_3, 128, [3, 3], scope='conv3_2')
                pool0_3 = slim.max_pool2d(conv0_3, [2, 2], scope='pool3')
                conv0_4 = slim.conv2d(pool0_3, 256, [3, 3], scope='conv4_1')


        fc_in = tf.layers.flatten(conv0_4)
        ...
        with tf.variable_scope('conv'):
            with slim.arg_scope([slim.conv2d], normalizer_fn=None,
                ...
                conv4 = tf.concat([conv4, conv0_4], axis=3)
        ...
        with tf.variable_scope('conv_transpose'):
            with slim.arg_scope([slim.conv2d, slim.conv2d_transpose], normalizer_fn=None,
                ...
                up5 = tf.concat([up5, conv3, conv0_3], axis=3)
                ...
                up6 = tf.concat([up6, conv2, conv0_2], axis=3)
                ...
                up7 = tf.concat([up7, conv1, conv0_1], axis=3)
    ...
    def compute_loss(self, net_out, logits):
        ...
        loss1 = tf.reduce_mean(tf.abs(net_out-self._image_ab))
