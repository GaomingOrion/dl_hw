...
tf.app.flags.DEFINE_integer("early_stop", 3,
                            "early-stopping")
tf.app.flags.DEFINE_string("model_path", "../../white_model/adv_train/fmnist_cnn.ckpt",
...
if __name__ == "__main__":
    ...
    # Load dataset
    d = Fashion_MNIST(data_dir='../../data')
    # 添加对抗样本到训练集中
    with open('../../taskD_result/adv_data.pkl', 'rb') as f:
        adv_imgs, adv_labels = pickle.load(f)
    adv_imgs = numpy.concatenate([d.train.images, adv_imgs])
    adv_labels = numpy.concatenate([d.train.labels, adv_labels])
    d.train = dataset(adv_imgs, adv_labels)
